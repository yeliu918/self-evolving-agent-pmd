/* =============================================================================
 * data.js — all recorded/scripted content for the MemCurator demo.
 ...
 *
 * Numbers: MemCurator draft (Tables 1–2). ± std in `std`. Illustrative payload
...
window.DEMO_METHODS = [
  ...
  {
    id: "memcurator", title: "MemCurator", timing: "read",
...
/* -----------------------------------------------------------------------------
 * Results (Tab 4). Values from MemCurator draft Tables 1–2.
...
window.DEMO_RESULTS = {
  alfworld: {
    ...
    note: "SkillOS and MemCurator are each shown twice — with a frozen (untrained) curator and an RL-trained curator — so base compares to base and trained to trained.",
    executors: {
      "Qwen3-8B": [
        ...
        { method: "MemCurator · frozen", value: 60.5, std: 2.6, family: "ours-base" },
        { method: "MemCurator · RL",     value: 77.4, std: 2.9, family: "ours" },
      ],
      "Gemini-2.5-Pro": [
        ...
        { method: "MemCurator · frozen", value: 80.0, std: 1.5, family: "ours-base" },
        { method: "MemCurator · RL",     value: 86.2, std: 1.9, family: "ours" },
      ],
      "GPT-5.4": [
        ...
        { method: "MemCurator · frozen", value: 79.3, std: 3.6, family: "ours-base" },
        { method: "MemCurator · RL",     value: 86.7, std: 0.7, family: "ours" },
      ],
    },
  },
  webshop: {
    ...
    executors: {
      "Qwen3-8B": [
        ...
        { method: "MemCurator · frozen", value: 11.7, std: 0.5, alt: 32.5, family: "ours-base" },
        { method: "MemCurator · RL",     value: 32.8, std: 1.7, alt: 61.1, family: "ours" },
      ],
      "Gemini-2.5-Pro": [
        ...
        { method: "MemCurator · frozen", value: 44.4, std: 0.6, alt: 54.7, family: "ours-base" },
        { method: "MemCurator · RL",     value: 50.5, std: 0.8, alt: 61.0, family: "ours" },
      ],
      "GPT-5.4": [
        ...
        { method: "MemCurator · frozen", value: 39.3, std: 0.7, alt: 49.9, family: "ours-base" },
        { method: "MemCurator · RL",     value: 45.4, std: 0.0, alt: 53.8, family: "ours" },
      ],
    },
  },
  tau2: {
    ...
    note: "Executor: GPT-5.4 (the only τ² block in the draft). MemCurator shown with a Qwen curator and a GPT curator. Per-domain SR on hover.",
    executors: {
      "GPT-5.4": [
        ...
        { method: "MemCurator · Qwen cur.",value: 68.3, std: 3.2, dom: { Airline: 66.0, Retail: 81.1, Telecom: 57.9 }, family: "ours-base" },
        { method: "MemCurator · GPT cur.", value: 73.4, std: 2.0, dom: { Airline: 63.5, Retail: 84.0, Telecom: 72.6 }, family: "ours" },
      ],
    },
  },
};
...
window.DEMO_SECONDARY = {
  ...
  efficiency: {
    ...
    rows: [
      ...
      { method: "MemCurator-base", input: 10.9, output: 1.00, steps: 13.2 },
      { method: "MemCurator",      input: 9.8,  output: 0.87, steps: 11.6 },
    ],
  },
  ablations: {
    ...
    base: {
      caption: "Training-free curator (MemCurator-base) — Figure 2.",
      rows: [
        { variant: "MemCurator-base (full)",          claim: "—", alf: 60.5, ws: 11.7, full: true },
    ...
    rl: {
      caption: "RL-trained curator (MemCurator) — Figure 3.",
      rows: [
        { variant: "MemCurator (full)",       claim: "—", alf: 77.4, ws: 32.8, full: true },
        ...
        { variant: "MemCurator-base (no RL)", claim: "RL compounds the read-time advantage", alf: 60.5, ws: 11.7 },
      ],
    },
  },
};
...
window.DEMO_CALLOUTS = [
  {
    ...
    body: "At a matched frozen executor and comparable curator size, MemCurator outperforms SkillOS on every benchmark — learned read-time curation dominates learned write-time.",
    evidence: "ALFWorld SR (Qwen3-8B): MemCurator 77.4 vs SkillOS 61.2.",
  },
  {
    title: "Structure first, RL compounds",
    body: "The UNTRAINED curator (MemCurator · frozen) already beats every write-time baseline — task-adaptive reframing is the primary source of gains, not curator training. RL then adds a further jump.",
   ...
   "examples": [
    {
     ...
     "methods": {
      "memcurator": {
       ...
       "reward": 1.0,
       "success": true,
       "summary": "MemCurator's briefing was written for THIS ticket — it lays out the exact telecom triage order (identify the line → rule out device causes → check suspension/billing), so the frozen agent follows it and passes every evaluation check."
      },
      "reasoningbank": {
       "label": "ReasoningBank",
       "injected": {
  ...
  "examples": [
   {
    ...
    "methods": {
     "memcurator": {
      ...
      "reward": 1.0,
      "success": true,
      "summary": "MemCurator's briefing was written for THIS task — it named the search query and the exact product to check, so the agent bought the right item fast."
     },
     "reasoningbank": {
      "label": "ReasoningBank",
      "injected": {
  ...
  "examples": [
   {
    ...
    "methods": {
     "memcurator": {
      ...
      "reward": 1.0,
      "success": true,
      "summary": "MemCurator's briefing laid out the clean-then-place plan for this task — the agent found the ladle, cleaned it, and placed it in a handful of turns."
     },
     "skillos": {
      "label": "SkillOS",
      "injected": {
   ...
   },
   {
    ...
    "methods": {
     "memcurator": {
      ...
      "reward": 1.0,
      "success": true,
      "summary": "MemCurator's briefing pinpointed where the candle was and where to place it — solved quickly."
     },
     "skillos": {
      "label": "SkillOS",
      "injected": {
   ...
   },
   {
    ...
    "methods": {
     "memcurator": {
      ...
      "reward": 1.0,
      "success": true,
      "summary": "MemCurator's briefing gave the heat-then-place recipe (find mug → heat → place) — the agent finished cleanly."
     },
     "skillos": {
      "label": "SkillOS",
      "injected": {
